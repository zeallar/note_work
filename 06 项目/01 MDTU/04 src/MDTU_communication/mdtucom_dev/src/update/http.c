...
Author:zhouBL
Version:
Description:http下载类方法
Others:
created date:2023/11/7 3:15 下午
modified date:
*************************************************/
...
typedef struct {
    int32_t sock;                       //与服务器通信的socket
    FILE *in;                       //sock描述符转为文件指针，方便读写
    int8_t host_name[HOST_NAME_LEN];  //主机名
    int32_t port;                       //主机端口号
    int8_t uri[URI_MAX_LEN];          //资源路径
    int8_t buffer[RECV_BUF];          //读写缓冲
    int32_t status_code;                //http状态码
    int32_t chunked_flag;               //chunked传输的标志位
    int32_t len;                        //Content-length里的长度
    int8_t location[URI_MAX_LEN];     //重定向地址
    int8_t *save_path;                //保存内容的路径指针
    FILE *save_file;                //保存内容的文件指针
    int32_t recv_data_len;              //收到数据的总长度
...
static int32_t print_level = /*MSG_DEBUG |*/ MSG_INFO | MSG_ERROR;
...
/* 不区分大小写的strstr */
int8_t *strncasestr(int8_t *str, int8_t *sub)
{
    if(!str || !sub)
        return NULL;
...
int32_t parser_URL(int8_t *url, http_t *info)
{
    int8_t *tmp = url, *start = NULL, *end = NULL;
    int32_t len = 0;
...
unsigned long dns(int8_t* host_name)
{
...
int32_t set_socket_option(int32_t sock)
{
    ...
    // 设置接送超时
    if(-1 == setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, (int8_t *)&timeout, 
            sizeof(struct timeval)))
    {
        lprintf(MSG_ERROR, "setsockopt error: %m\n");
        return -1;
...
/* 连接到服务器 */
int32_t connect_server(http_t *info)
{
    int32_t sockfd;
...
/* 发送http请求 */
int32_t send_request(http_t *info)
{
    int32_t len;
...
/* 解析http头 */
int32_t parse_http_header(http_t *info)
{
    int8_t *p = NULL;
...
/* 保存服务器响应的内容 */
int32_t save_data(http_t *info, const int8_t *buf, int32_t len)
{
    int32_t total_len = len;
    int32_t write_len = 0;
    ...
    while(total_len)
    {
        write_len = fwrite(buf, sizeof(int8_t), len, info->save_file);
...
/* 读数据 */
int32_t read_data(http_t *info, int32_t len)
{
    int32_t total_len = len;
    int32_t read_len = 0;
    int32_t rtn_len = 0;
    ...
    while(total_len)
    {
        ...
        rtn_len = fread(info->buffer, sizeof(int8_t), read_len, info->in);
        if(rtn_len < read_len)
        {
            if(ferror(info->in))
            {
...
/* 接收服务器发回的chunked数据 */
int32_t recv_chunked_response(http_t *info)
{
    long part_len;
    ...
    do{
        ...
        //读走后面的\r\n两个字符
        if(2 != fread(info->buffer, sizeof(int8_t), 2, info->in))
        {
...
/* 接收服务器的响应数据 */
int32_t recv_response(http_t *info)
{
    int32_t len = 0, total_len = info->len;
...
/* 下载主函数 */
int32_t http_download(int8_t *url, int8_t *save_path)
{
	/*判断目录是否存在，若不存在,则创建*/
	if ( access(TEMP_IMAGE_PATH, F_OK) != 0)
		mkdir(TEMP_IMAGE_PATH,0755);
	
    http_t *info = NULL;
    int8_t tmp[URI_MAX_LEN] = {0};
